...
import { createPeerStorage, type AsyncPeerStorage, type PeerStorage } from '../libs/adapters/inmemory.js';
import {
  activeSignalEntries,
  buildEdgeShape,
  extractPeerSignals,
...
// ── Warning messages ──────────────────────────────────────────
const LICENSE_WARN =
  '[peer-devicer] No license key — running on the free tier ' +
...
const LICENSE_INVALID_WARN =
  '[peer-devicer] License key could not be validated — falling back to the free tier. ' +
  'Check your key or network connectivity.';
const DEVICE_LIMIT_WARN =
  `[peer-devicer] Free-tier device limit reached (${FREE_TIER_MAX_DEVICES.toLocaleString()} devices). ` +
 ...
 * ```
 */
export class PeerManager implements DeviceManagerPlugin {
  private storage: PeerStorage | AsyncPeerStorage;
  ...
  private readonly _customStorage: boolean;
  ...
  private async _doInit(): Promise<void> {
    ...
    if (!info.valid) {
      ...
      if (this.options.maxPeersPerDevice > FREE_TIER_MAX_HISTORY) {
        ...
        (this.options as { maxPeersPerDevice: number }).maxPeersPerDevice =
          FREE_TIER_MAX_HISTORY;
      }
    }
  }
  ...
  async assess(
    ...
    const isKnownDevice = (await Promise.resolve(
      this.storage.getEdges(deviceId, 1),
    )).length > 0;
    ...
    const signalEntries = activeSignalEntries(
      signals,
      this.options.enabledEdgeTypes,
      isFreeTier,
    );

    const peerDeviceIds = new Set<string>();
    const newEdgeParts: Array<{ peerDeviceId: string; edgeType: PeerEdgeType; signalValue: string }> = [];
    ...
    for (const { edgeType, signalValue } of signalEntries) {
      ...
      const found = await Promise.resolve(
        this.storage.registerDeviceSignal(deviceId, edgeType, signalValue),
      );
    ...
    // ── Load peer caches ───────────────────────────────────────
    const existingEdges = await Promise.resolve(
      this.storage.getEdges(deviceId, this.options.maxPeersPerDevice),
    );
    ...
    const peersWithEdge: PeerWithEdge[] = [];
    for (const peerId of uniquePeerIds) {
      const peerEdges = existingEdges.filter(
        (e) => e.deviceIdA === peerId || e.deviceIdB === peerId,
      );
      const strongestEdge = peerEdges.sort((a, b) => b.weight - a.weight)[0];
      if (!strongestEdge) continue;

      const cache = await Promise.resolve(
        this.storage.getDeviceCache(peerId),
      );
    ...
    const deviceCache: PeerDeviceCache = {
      deviceId,
      updatedAt:      new Date(),
  ...
  registerWith(deviceManager: DeviceManagerLike): (() => void) | void {
    return deviceManager.registerIdentifyPostProcessor?.(
      PLUGIN_NAME,
      async ({ result, context, incoming }) => {
        const ctx = (context ?? {}) as PeerIdentifyContext;
        ...
        const reputation = await this.assess(
   ...
   */
  async getEdges(deviceId: string, limit?: number): Promise<PeerEdge[]> {
   ...
   */
  async getDeviceCache(deviceId: string): Promise<PeerDeviceCache | null> {
   ...
   * of pruned edges.
   */
  async pruneStaleEdges(olderThanMs: number): Promise<number> {
