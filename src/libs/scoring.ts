 ...
 */
export function computePeerReputation(
  peers: PeerWithEdge[],
  signals: PeerSignals,
  deviceId: string,
  ...
  for (const { edge, cache } of peers) {
    ...
    const peerSuspicion =
      ipRisk        * PEER_SUSPICION_WEIGHTS.ipRisk +
...
export function computeConfidenceBoost(
  summary: { taintScore: number; trustScore: number; isNewDevice: boolean },
  weight = 0.2,
): number {
  if (summary.isNewDevice) return 0;
