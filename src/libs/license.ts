// ────────────────────────────────────────────────────────────
//  Polar.js license validation for peer-devicer
 ...
 *
 * | Tier         | Price        | Device limit | Edge types        |
 * |--------------|-------------|--------------|-------------------|
 * | `free`       | $0/mo        | 10,000       | user + subnet     |
 * | `pro`        | $49/mo       | Unlimited    | all 5 types       |
 * | `enterprise` | $299/mo      | Unlimited    | all 5 + custom    |
 */
...
/** Maximum unique devices tracked on the free tier. */
export const FREE_TIER_MAX_DEVICES = 10_000;

/**
 * Maximum peer edge history depth per device on the free tier.
 * Pro/Enterprise default to 50 (configurable via `maxPeersPerDevice`).
 */
...
//
//  Name: "peer-devicer Pro"
//    Type: Subscription  — $49 / month
//    Benefit: License Keys  (activation limit: 1 — enforces single-server)
//
//  Name: "peer-devicer Enterprise"
//    Type: Subscription  — $299 / month
//    Benefit: License Keys  (activation limit: unlimited)
//
//  Note: all devicer plugins share the same Polar organisation.
//  You may reuse the same products and benefit IDs across all packages
 ...
 * All devicer plugins share the same organisation and products — paste
 * benefit IDs once and every library uses them.
 */
